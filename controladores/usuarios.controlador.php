	...
	if(isset($_POST["nuevoItem"])){
		...
		if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevoItem"]) &&
		   preg_match('/^[a-zA-Z0-9 ]+$/', $_POST["nuevoCosto"])){
			...
			if(isset($_FILES["nuevaFoto1"]["tmp_name"])){

				list($ancho, $alto)= getimagesize($_FILES["nuevaFoto1"]["tmp_name"]);
				...
				$directorio="vistas/img/portafolio/". $_POST["nuevoItem"];
				mkdir($directorio, 0755);
				// de acuerdo al tipo de imagen se aplicaran las siguietnes funciones por defecto de php
				...
				if($_FILES["nuevaFoto1"]["type"] == "image/jpeg"){
					...
					$ruta = "vistas/img/usuarios/". $_POST["nuevoItem"]."/".$aleatorio.".jpg";
					
					$origen = imagecreatefromjpeg($_FILES["nuevaFoto1"]["tmp_name"]);
				...
				if($_FILES["nuevaFoto1"]["type"] == "image/png"){
					...
					$ruta = "vistas/img/portafolio/". $_POST["nuevoItem"]."/".$aleatorio.".png";
					
					$origen = imagecreatefrompng($_FILES["nuevaFoto1"]["tmp_name"]);
			...
			$datos = array("item" =>$_POST["nuevoItem"],
						   "costo" =>$_POST["nuevoCosto"],
						   "Tipo" =>$_POST["nuevoTipo"],
						"foto"=>$ruta);

			$respuesta = ModeloUsuarios :: mdlIngresarItem($tabla, $datos);
